...
# We build the connection string at runtime so the build phase can use a
# dummy URL for Prisma schema validation without needing a live database.
# Railway PostgreSQL requires sslmode=require for secure connections.
...
if [ -z "$DATABASE_URL" ] || [ "$DATABASE_URL" = "postgresql://dummy:dummy@localhost:5432/dummy" ]; then
  if [ -z "$PGHOST" ] || [ -z "$PGPORT" ] || [ -z "$PGUSER" ] || [ -z "$PGPASSWORD" ] || [ -z "$PGDATABASE" ]; then
      echo "ERROR: DATABASE_URL not set and PG* vars missing (PGHOST, PGPORT, PGUSER, PGPASSWORD, PGDATABASE)"
          exit 1
            fi

              export DATABASE_URL="postgresql://${PGUSER}:${PGPASSWORD}@${PGHOST}:${PGPORT}/${PGDATABASE}?sslmode=require"
                echo "INFO: DATABASE_URL constructed from PG* vars (host=${PGHOST}, db=${PGDATABASE})"
                else

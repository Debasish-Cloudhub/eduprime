// Server component — fetches real course data at SSR time
...
async function fetchCourses(): Promise<Course[]> {
  try {
    const res = await fetch(`${API_URL}/public/courses?limit=8&page=1`, {
...
// Static fallback if DB has no courses
      ...
      <div className="max-w-7xl mx-auto px-4">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-500 mt-2 max-w-xl mx-auto">
            UGC-approved distance and online programs from top universities
          </p>
        </div>
        ...
        {courses.length > 0 ? (
          <>
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 text-xs text-gray-400 mb-4">
                      ...
                      {course.stream && (
                        <span className="flex items-center gap-1">
                          <BookOpen className="w-3 h-3" />{course.stream}
                        </span>
                      )}
                    </div>
                    ...
                    {course.fees > 0 && (
                      <div className="text-sm font-semibold text-gray-700 mb-4">
                        {symbol}{Number(course.fees).toLocaleString()} / year
                      </div>
                    )}

                    <div className="flex items-center gap-1 font-semibold text-sm text-blue-700 group-hover:gap-2 transition-all">
            ...
            <div className="text-center mt-10">
              <Link
                href="/courses"
                className="inline-flex items-center gap-2 px-8 py-3.5 bg-blue-700 text-white font-bold rounded-xl hover:bg-blue-800 transition-all shadow-lg"
              >
          ...
          <>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {FALLBACK_COURSES.map(({ name, desc, duration, mode }) => (
                <Link
                  key={name}
                  href="/courses"
                  className="block bg-white rounded-2xl border border-gray-100 p-6 hover:shadow-xl transition-all group cursor-pointer overflow-hidden relative"
                >
                  ...
                  <div className="flex items-center gap-1 font-semibold text-sm text-blue-700">
